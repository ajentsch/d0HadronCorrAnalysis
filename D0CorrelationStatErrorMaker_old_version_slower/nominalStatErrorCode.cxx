...
//constants///////////////////////////////////////
    ...
    TFile * inputFile = new TFile("singles_dist_corrected_phi_bins_part1.root");
    TFile * outputFile = new TFile("output_errors.root", "RECREATE");
    ...
    double firstDelPhiBin = -TMath::PiOver2() + phiBinShift +.5*(delPhiBinWidthCorr);
    
    for(int i = 1; i < NUM_PHI_CORR_BINS+1; i++){
    
        delPhiCorrBinCenters[i] = firstDelPhiBin + (i-1)*delPhiBinWidthCorr;
        ...
        //cout << endl << "Eta Z values: " << endl << endl;
        for(int y = 9; y > 0; y--){
        ...
        //cout << endl << "Phi Z values: " << endl << endl;
        for(int y = 24; y > 0; y--){
    ...
    double zEtaDBinCenter;
    double zEtaHBinCenter;
    double delEtaStripe;
    double delEtaStripeMirror;
    
    double zPhiDBinCenter;
    double zPhiHBinCenter;
    double delPhiStripe;
    double delPhiStripeMirror;
    ...
    for(int band = 1; band < 2; band++){
        for(int ptBin = 1; ptBin < 2; ptBin++){
            for(int vzBin = 5; vzBin < 6; vzBin++){
                for(int multBin = 12; multBin < 13; multBin++){
                                ...
                                for(int etaHBin = 1; etaHBin < NUM_ETA_PRIM_BINS+1; etaHBin++){
                                    for(int phiDBin = 1; phiDBin < NUM_PHI_PRIM_BINS+1; phiDBin++){
                                        for(int phiHBin = 1; phiHBin < NUM_PHI_PRIM_BINS+1; phiHBin++){
                                            ...
                                            zPhiDBinCenter = zPhiHist->GetXaxis()->GetBinCenter(phiDBin);  //gets phi value for D bin center
                                            zPhiHBinCenter = zPhiHist->GetYaxis()->GetBinCenter(phiHBin);  //gets phi value for H bin center
                                            delPhiStripe = zPhiDBinCenter - zPhiHBinCenter;  //gets absolute delPhi from those values
                                            //delPhiStripeMirror = -delPhiStripe;             //gets the negative value stripe for the symmetric delphi bin

                                            //cout << "original value delPhiStripe: " << delPhiStripe << endl;
                                            
                                            //if(delPhiStripe < -TMath::Pi()) { delPhiStripe = delPhiStripe + 2*TMath::Pi(); } //shifts [-2pi, 2pi] -> [-pi, pi]
                                            //else if(delPhiStripe >  TMath::Pi()) { delPhiStripe = delPhiStripe - 2*TMath::Pi(); } //shifts [-2pi, 2pi] -> [-pi, pi]
                                            
                                            if(isSumLessThanZero(delPhiStripe, TMath::Pi(), .00001)) { delPhiStripe = delPhiStripe + 2*TMath::Pi(); } //shifts [-2pi, 2pi] -> [-pi, pi]
                                            else if(isDiffGreaterThanZero(delPhiStripe, TMath::Pi(), .00001)) { delPhiStripe = delPhiStripe - 2*TMath::Pi(); } //shifts [-2pi, 2pi] -> [-pi, pi]
                                            
                                            //cout << "shifted value delPhiStripe [-2pi, 2pi] -> [-pi,pi] : " << delPhiStripe << endl;
                                            
                                            delPhiStripe = TMath::Abs(delPhiStripe);
                                            //cout << "shifted value delPhiStripe (absolute value) [-2pi, 2pi] -> [-pi,pi] : " << delPhiStripe << endl;
                                            //if(delPhiStripe > (3*TMath::PiOver2()+phiBinShift)) { delPhiStripe = delPhiStripe - 2*TMath::Pi(); } 
                                            if(isDiffGreaterThanZero(delPhiStripe, 3*TMath::PiOver2()+phiBinShift, .00001) || isDiffZero(delPhiStripe, 3*TMath::PiOver2()+phiBinShift, .00001)) { delPhiStripe = delPhiStripe - 2*TMath::Pi(); } 
                                            
                                            //cout << "pi/2 shift to line up 0 and pi : " << delPhiStripe << endl;

                                            
                                            if( isZero(delPhiStripe, .0001) ) { delPhiStripeMirror == 0.0; }
                                            else delPhiStripeMirror = -delPhiStripe;
                                            
                                            //cout << "delPhiMirror: " << delPhiStripeMirror << endl << endl;
                                            
                                            //if(delPhiStripeMirror < (-TMath::PiOver2()+phiBinShift)) { delPhiStripeMirror = delPhiStripeMirror + 2*TMath::Pi(); } 
                                            if(isSumLessThanZero(delPhiStripeMirror, TMath::PiOver2()+phiBinShift, .00001) || isDiffZero(delPhiStripeMirror, -TMath::PiOver2()+phiBinShift, .00001)) { delPhiStripeMirror = delPhiStripeMirror + 2*TMath::Pi(); } 
                                            
                                            //cout << "delPhiMirror after 0 and pi shift: " << delPhiStripeMirror << endl;
                                            
                                            if(!(isDiffZero(delPhiStripe, delPhiValue, .0001)) && !(isDiffZero((delPhiStripe - phiBinShift), delPhiValue, .0001)) && !(isDiffZero((delPhiStripe + phiBinShift), delPhiValue, .0001)) &&
                                               !(isDiffZero(delPhiStripeMirror, delPhiValue, .0001)) && !(isDiffZero((delPhiStripeMirror - phiBinShift), delPhiValue, .0001)) && !(isDiffZero((delPhiStripeMirror + phiBinShift), delPhiValue, .0001))) {continue;}
                                            
                                            //cout << "phiDBin: " << phiDBin << "    phiHBin: " << phiHBin << endl;
                                            //cout << "final value of delphi strip: " << delPhiStripe <<",    Final value of mirror: " << delPhiStripeMirror << endl;
                                            ...
                                            //cout << "etaDBin: " << etaDBin << "    etaHBin: " << etaHBin << "    phiDBin: " << phiDBin << "    phiHBin: " << phiHBin << endl;
                                            //cout << "mBar: " << mBar << "    nBar: " << nBar << "   Zeta: " << etaZValues[etaDBin][etaHBin] << "   Zphi: " << phiZValues[phiDBin][phiHBin] << endl;
                                            //cout << "NEv: " << NEv[band][ptBin][vzBin][multBin] << endl;
                                                ...
                                                for(int etaHBinPrime = 1; etaHBinPrime < NUM_ETA_PRIM_BINS+1; etaHBinPrime++){
                                                    for(int phiDBinPrime = 1; phiDBinPrime < NUM_PHI_PRIM_BINS+1; phiDBinPrime++){
                                                        for(int phiHBinPrime = 1; phiHBinPrime < NUM_PHI_PRIM_BINS+1; phiHBinPrime++){
                                                            ...
                                                            zPhiDBinCenter = zPhiHist->GetXaxis()->GetBinCenter(phiDBinPrime);  //gets phi value for D bin center
                                                            zPhiHBinCenter = zPhiHist->GetYaxis()->GetBinCenter(phiHBinPrime);  //gets phi value for H bin center
                                                            delPhiStripe = zPhiDBinCenter - zPhiHBinCenter;                     //gets delPhi from those values
                                                            //delPhiStripeMirror = -delPhiStripe;             //gets the negative value stripe for the symmetric delphi bin

                                                            //all of this business checks to make sure the "stripe" value will land in the center or edge of a real bin on the correlation
                                                            if(isSumLessThanZero(delPhiStripe, TMath::Pi(), .00001)) { delPhiStripe = delPhiStripe + 2*TMath::Pi(); } //shifts [-2pi, 2pi] -> [-pi, pi]
                                                            else if(isDiffGreaterThanZero(delPhiStripe, TMath::Pi(), .00001)) { delPhiStripe = delPhiStripe - 2*TMath::Pi(); } //shifts [-2pi, 2pi] -> [-pi, pi]
                                                            ...
                                                            //cout << "delPhiMirror: " << delPhiStripeMirror << endl;
                                                            ...
                                                            term2 = term2 + 2*NEv[band][ptBin][vzBin][multBin]*NMix*NMix*etaZValues[etaDBin][etaHBin]*phiZValues[phiDBin][phiHBin]*etaZValues[etaDBinPrime][etaHBin]*phiZValues[phiDBinPrime][phiHBin]*mBar*mBarPrime*nBar;
                                            ...
                                            //After this, we have finished a whole calculation for one value of a possible pair of z-bins
