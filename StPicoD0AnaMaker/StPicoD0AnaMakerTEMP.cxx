        ...
        if(kPiCharge < 0){// begin US conditional
            ...
            if(bandBin > -1){// begin sibling pair formation with whatever "band" we are in, if we have a candidate in the band -- NEED TO MAKE A TRACK LIST TO SPEED THIS UP!!!
                ...
                for(unsigned int i = 0; i < mAssociatedHadronList.size(); i++){ // begin picoDST loop for d0-hadron correlations
                    ...
                    delPhi = kp->phi()-phi;
                    delEta = kp->eta()-eta; //ABSOLUTE VALUE FOR SYMMETERIZATION
                    
                    if((delEta < .1 || delEta > -.1) && USE_TWO_TRACK_INEFF && !(passTwoTrackCuts(daughterKaonMom, kaon->charge(), trackMom, mAssociatedHadronListCharge[i], bField) || 
                   ...
                   /* delPhi = TMath::Abs(delPhi);//Gets absolute value of angle 
                    ...
                    if(delPhi > (3*TMath::PiOver2()) + phiBinShift){ delPhi = delPhi - 2*TMath::Pi(); }
                    else if(delPhi < -TMath::PiOver2() + phiBinShift){ delPhi = delPhi + 2*TMath::Pi(); }                   
 
                    //efficiencyCorrInfoSib[ptBin][centralityClass]->Fill(2.5); // increments the pair count
                
                    if(D0_HADRON_CORR){
                    
                        hadronEffWeight = effWeightPions->Eval(pt, 0, 0, 0);                      //E hadron
                        pionEffWeight   = effWeightPions->Eval(daughterPionMom.perp(), 0, 0, 0);  //E Pion
                        kaonEffWeight   = effWeightKaons->Eval(daughterKaonMom.perp(), 0, 0, 0);  //E Kaon
                        D0Eff = effWeightD0[centralityClass]->Eval(kp->pt(), 0, 0, 0);            //E K-Pi pair
                        
                        tripleProduct = hadronEffWeight*pionEffWeight*kaonEffWeight;
                    ...
                    for (int idx = 0; idx < nD0Tracks; idx++){//begin loop over event D0 kaon-pion list
                        ...
                        if(kaonPionTrack1.charge() < 0 && kaonPionTrack1.mass() > D0InvMassLow && kaonPionTrack1.mass() < D0InvMassHigh)                 { bandBin = 1; }    //US center band
                            ...
                            for(int j = 0; j < nHadronTracks; j++){//begin loop over hadron event tracks
                               ...
                               // delPhiCp = -delPhi;//Gets negative copy of absolute value of angle.....
                                if(delPhi < -TMath::PiOver2() + phiBinShift){ delPhi = delPhi + 2*TMath::Pi(); }//....and shifts it
                                //if(delPhiCp < -TMath::PiOver2()){ delPhiCp = delPhiCp + 2*TMath::Pi(); }//....and shifts it
                
                
                                delEta = kaonPionTrack1.gMom().pseudoRapidity()-eta;
                                
                                if((delEta < .1 || delEta > -.1) && USE_TWO_TRACK_INEFF && !(passTwoTrackCuts(daughterKaonMom, kaonPionTrack1.kaonCharge(), trackMom, hadronTrack2.charge(), hadronBField) || 
                                                            passTwoTrackCuts(daughterKaonMom, kaonPionTrack1.pionCharge(), trackMom, hadronTrack2.charge(), hadronBField)) ){ continue; }
               
                                //efficiencyCorrInfoMix[ptBin][centralityClass]->Fill(2.5); // increments the pair count
                
                                hadronEffWeight = effWeightPions->Eval(pt, 0, 0, 0);
                                pionEffWeight   = effWeightPions->Eval(daughterPionPt, 0, 0, 0);
                                kaonEffWeight   = effWeightKaons->Eval(daughterKaonPt, 0, 0, 0);
                                D0Eff = effWeightD0[centralityClass]->Eval(D0Pt, 0, 0, 0);
                        
                                //efficiencyCorrInfoMix[ptBin][centralityClass]->Fill(.5, backgroundEffWeight);
                                //efficiencyCorrInfoMix[ptBin][centralityClass]->Fill(1.5, D0HadronEffWeight);
