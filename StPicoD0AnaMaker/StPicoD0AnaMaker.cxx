...
Int_t StPicoD0AnaMaker::Init()
{
    ...
    SINGLES_DISTS       = false;         //sets the output for "singles" distributions, which are the 2d-histograms on the eta and phi of the D0 and hadron, rather than delEta/delPhi -- default is FALSE
    D0_HADRON_CORR      = true;          //sets the output for the correlations (turn off for testing other things) -- default is TRUE
    EVENT_MIXING        = true;          //toggle events mixing on and off -- default is TRUE
    
    USE_SYMMETERIZATION = true;
    ...
    SOFT_MIS_PID_PROTECTION       = false; //softer version of the double-MIS-PID protection - throws out all two-D0 events
   
    USE_PAIR_WISE_PT_CUT = false;          //obsolete cut -- will remove later.
    USE_PAIR_WEIGHT      = true;
    
    TURN_OFF_LOW_PT      = true;
    //----------------------------------------------------------//
    ...
    NUM_ETA_BINS       = 13;                 //number of delEta bins to use for correlations
    ...
    NUM_PT_BINS        = 5;                 //number of pt bins used for analysis -- narrow bins down low (more stats), wider bins up high.
    NUM_D0_CUT_PT_BINS = 6;                 //same as above, just includes an "integrated" bin --  this was a screw-up that should have been addressed.
    ...
    d0DaughterPionPtMin = .15;   // NOMINAL HAS BEEN .15 UP TO THIS POINT  2/9/2017
    d0DaughterKaonPtMin = .15;   // NOMINAL HAS BEEN .15 UP TO THIS POINT
    
    D0InvMassLow        = 1.82; //signal region US invariant mass Low cut
    D0InvMassHigh       = 1.90; //signal region US invariant mass High cut
    
    USSideBandLeftLow   = 1.7; //1.7 //left sideband low cut
    USSideBandLeftHigh  = 1.8; //1.8 //left sideband high cut
    ...
    //pt = 2-3 GeV/c
    d0TopologicalCutArray[2][0] = .0212; //minimum decay length
    d0TopologicalCutArray[2][1] =  .007; //.0057; //maximum DCA daughters
    d0TopologicalCutArray[2][2] = .0038; //maximum DCA between d0 vector and PV 
    d0TopologicalCutArray[2][3] = .0086; //minimum DCA between Pi and PV
    d0TopologicalCutArray[2][4] = .0095; //minimum DCA between K and PV
    
    /* //pt = 3-4 GeV/c
    d0TopologicalCutArray[3][0] = .0212; //minimum decay length
    d0TopologicalCutArray[3][1] = .0057; //maximum DCA daughters
    d0TopologicalCutArray[3][2] = .0038; //maximum DCA between d0 vector and PV 
    d0TopologicalCutArray[3][3] = .0086; //minimum DCA between Pi and PV
    d0TopologicalCutArray[3][4] = .0095; //minimum DCA between K and PV
    
     //pt = 4-10 GeV/c
    d0TopologicalCutArray[4][0] = .0212; //minimum decay length
    d0TopologicalCutArray[4][1] = .0057; //maximum DCA daughters
    d0TopologicalCutArray[4][2] = .0038; //maximum DCA between d0 vector and PV 
    d0TopologicalCutArray[4][3] = .0086; //minimum DCA between Pi and PV
    d0TopologicalCutArray[4][4] = .0095; //minimum DCA between K and PV*/
    
    //pt = 3-5 GeV/c --> trying 3-4GeV
    ...
    //pt = 5-10 GeV/c --> trying 4-10GeV
    d0TopologicalCutArray[4][0] = .0240; //minimum decay length
    d0TopologicalCutArray[4][1] = .0045; //maximum DCA daughters
    d0TopologicalCutArray[4][2] = .0035; //maximum DCA between d0 vector and PV 
    d0TopologicalCutArray[4][3] = .0062; //minimum DCA between Pi and PV
    d0TopologicalCutArray[4][4] = .0058; //minimum DCA between K and PV  
    ...
    TString D0CutPtBinsLabel[6] = {"0-1", "1-2", "2-3", "3-4", "4-10", "Integrated"};   //NOMINAL VALUES -- DON'T DELETE -->changed the high pt bins
    ...
    TString DStarLabel = "DStarHistograms_";
	TString KPiPiMinusKPiLabel = "KPiPi_minus_KPi_";
    ...
    for(int k = 0; k < NUM_PT_BINS; k++){
        for(int j = 0; j < 3; j++){
            ...
            dStarHistogramsSib[k][j]    = new TH1D(str1, str1, 400, 1.8, 3.0);
            str1 = mixLabel + DStarLabel + PtBinLabel + binLabelPt[k] + binLabelCentralityClass[j];
            dStarHistogramsMix[k][j]    = new TH1D(str1, str1, 400, 1.8, 3.0);
			
			str1 = siblingLabel + KPiPiMinusKPiLabel + PtBinLabel + binLabelPt[k] + binLabelCentralityClass[j];
            dStarMinusD0HistogramsSib[k][j]    = new TH1D(str1, str1, 400, 0.135, .3);
            str1 = mixLabel + KPiPiMinusKPiLabel + PtBinLabel + binLabelPt[k] + binLabelCentralityClass[j];
            dStarMinusD0HistogramsMix[k][j]    = new TH1D(str1, str1, 400, 0.135, .3);
            
            str1 = siblingLabel + angularLabel + DStarLabel + PtBinLabel + binLabelPt[k] + binLabelCentralityClass[j];
            dStarAngularDistSib[k][j] = new TH2D(str1, str1, NUM_ETA_BINS, -2, 2, NUM_PHI_BINS, -TMath::PiOver2()+phiBinShift, (3*TMath::PiOver2())+phiBinShift);
            str1 = mixLabel + angularLabel + DStarLabel + PtBinLabel + binLabelPt[k] + binLabelCentralityClass[j];
            dStarAngularDistMix[k][j] = new TH2D(str1, str1, NUM_ETA_BINS, -2, 2, NUM_PHI_BINS, -TMath::PiOver2()+phiBinShift, (3*TMath::PiOver2())+phiBinShift);
   ...
   /* if(D0_HADRON_CORR){//begin D0-Hadron Correlation Histograms
    ...
    for(int band = 2; band < NUM_PT_BINS; band++){   //band is set to 2 to drop low pt
		for(int i = 0; i < 3; i++){
...
{ 
   ...
   for(int band = 2; band < NUM_PT_BINS; band++){   //band set to 2 to drop low pt
		for(int i = 0; i < 3; i++){
    ...
    for(int k = 0; k < NUM_PT_BINS; k++){
        for(int j = 0; j < 3; j++){
            ...
            dStarHistogramsSib[k][j]->Write();
            dStarHistogramsMix[k][j]->Write();
			dStarMinusD0HistogramsSib[k][j]->Write();    
			dStarMinusD0HistogramsMix[k][j]->Write();   
            dStarAngularDistSib[k][j]->Write();
            dStarAngularDistMix[k][j]->Write();
...
/***************************************************************************************************************/ 
    ...
    double dStarInvMass    = 0.0;
	double dStarPionMass   = 0.0;
    double kPiPiMass       = 0.0;
    ...
    std::vector <int> mAssociatedHadronListTPCPID; // 0 - is TPC pion, 1 - is TPC kaon , 3 - is doubly IDd (has nSigma for K and Pi)
...
/****************************************************************************************/  
    ...
    for (int idx = 0; idx < aKaonPion->GetEntries(); ++idx){//begin loop to check if event can be used (check if a D0 exists)
        ...
        if(TURN_OFF_LOW_PT && topologicalCutPtBin < 2) { continue; }
      
	    if(DEBUG) { cout << "bandBin: " << bandBin << endl; }
	  
        if(topologicalCutPtBin == -1 ) {continue;}
        
        if(USE_DOUBLE_MIS_PID_PROTECTION && (((fabs(kaon->nSigmaKaon()) <= 2.0) && (fabs(kaon->nSigmaPion()) <= 2.0)) &&   //checks if double mis-PID AND a two-D0 event
...
/*****************************************************************************************************************/  
   ...
   if(numD0s > 1 && SOFT_MIS_PID_PROTECTION) { return kStOk; }
   d0CountPerEvent->Fill(numD0s);
...
/****************************************************************************************************************/   
    ...
    for(unsigned int i = 0; i < picoDst->numberOfTracks(); ++i){ // Begin associated hadron list loop
        ...
        if(pt < .15 || pt > 15.45)                                                                        {continue;}  //check pt cut
        ...
        if(pt < hadronPtMin || pt > hadronPtMax)                                                                        {continue;}  //check pt cut ADDED IN
		...
		if((fabs(trk->nSigmaPion()) < 2.0) && (fabs(trk->nSigmaKaon()) >= 2.0)) { mAssociatedHadronListTPCPID.push_back(0); } //likely pion
		else if((fabs(trk->nSigmaKaon()) < 2.0) && (fabs(trk->nSigmaPion()) >= 2.0)) { mAssociatedHadronListTPCPID.push_back(1); } //likely kaon
		else mAssociatedHadronListTPCPID.push_back(3); //something else
    ...
    if(EVENT_MIXING){  // begin event mixing on/off switch
        ...
        if(storeEventToMix){ //Begin block to store non-d0 containing events 
            ...
            for(unsigned int i = 0; i < picoDst->numberOfTracks(); ++i){//begin loop to add picoDST tracks to buffer
                        ...
                        if((fabs(trk->nSigmaPion()) < 2.0) && (fabs(trk->nSigmaKaon()) >= 2.0)) { PIDflag = 0; } //likely pion
						else if((fabs(trk->nSigmaKaon()) < 2.0) && (fabs(trk->nSigmaPion()) >= 2.0)) { PIDflag = 1; } //likely kaon
						else PIDflag = 3; //something else
            
                        eventBufferPicoEvent[VzBin][centralityBin]->addTrackToEvent(eventBufferPicoEvent[VzBin][centralityBin]->getBufferIndex()-1, trackMom, trk->charge(), PIDflag);   //0 pion, 1 kaon, 3 other
...
/****************************************************************************************************************/
    ...
    for (int idx = 0; idx < aKaonPion->GetEntries(); ++idx){ // begin main sibling loop
        ...
        if(TURN_OFF_LOW_PT && ptBin < 2) {continue;}  //THIS IS TO SKIP ALL LOW-PT STUFF. DON'T FORGET THIS IS ON.
        ...
        if(kPiCharge < 0){// begin US conditional
            ...
            if(bandBin > -1){// begin sibling pair formation with whatever "band" we are in, if we have a candidate in the band -- NEED TO MAKE A TRACK LIST TO SPEED THIS UP!!!
                ...
                for(unsigned int i = 0; i < mAssociatedHadronList.size(); i++){ // begin picoDST loop for d0-hadron correlations
                    ...
                    delPhiRaw = kp->phi()-phi;
                    delEtaRaw = kp->eta()-eta;
                    delPhi = kp->phi()-phi;
                    ...
                    if(delPhiRaw < -TMath::Pi()) { delPhiRaw = delPhiRaw + 2*TMath::Pi(); }     //shift [-2Pi, 2Pi] -> [-Pi,Pi]
                    else if(delPhiRaw > TMath::Pi()) { delPhiRaw = delPhiRaw - 2*TMath::Pi(); }
                    
                    delPhi = TMath::Abs(delPhi);//Gets absolute value of angle
                    delPhiMinusPiPlusPi = delPhi;
                    
					if(delPhi >= (3*TMath::PiOver2()) + phiBinShift){ delPhi = delPhi - 2*TMath::Pi(); }//....and shifts it
                    if(delPhiRaw > (3*TMath::PiOver2()) + phiBinShift){ delPhiRaw = delPhiRaw - 2*TMath::Pi(); }
                    else if(delPhiRaw < -TMath::PiOver2() + phiBinShift){ delPhiRaw = delPhiRaw + 2*TMath::Pi(); }
                    ...
                    if(D0_HADRON_CORR){
                        ...
                        //tripleProduct = hadronEffWeight*HFTRatioPion*pionEffWeight*HFTRatioKaon*kaonEffWeight;
                        //doubleProduct = D0Eff*hadronEffWeight;
                        
                        if(tripleProduct <= 0 || doubleProduct <= 0) { continue; }
                       
                        if(bandBin == 1) { 
                        
                            finalPairWeight = calculatePairWeightSignalRegion(BOverBPlusS[ptBin][centralityClass], SOverBPlusS[ptBin][centralityClass], tripleProduct, doubleProduct, 
                                                                              siblingTripleEff[ptBin][centralityClass], siblingDoubleEff[ptBin][centralityClass]);
                        }                                                      
                        
                        else finalPairWeight = siblingTripleEff[ptBin][centralityClass]/tripleProduct;
                    
                        if(finalPairWeight <= 0) { continue; }
                        if(!USE_PAIR_WEIGHT) { finalPairWeight = 1; }
                    
                        kPiPiMass = calcInvMass3Particle(.493677, .139570 , .139570 , daughterKaonMom.x(), daughterKaonMom.y(), daughterKaonMom.z(), 
                                                          daughterPionMom.x(), daughterPionMom.y(), daughterPionMom.z(), trackMom.x(), trackMom.y(), trackMom.z()); 
                                                          
                        dStarPionMass = kPiPiMass - kp->m();
                            
                        if(bandBin == 1 && mAssociatedHadronListTPCPID[i] == 0 && ((pion->charge()*mAssociatedHadronListCharge[i]) == 1) 
                                        && dStarPionMass > 0.0 && dStarPionMass < .5 && kPiPiMass > 1.6 && kPiPiMass < 3.0){
                        
                            dStarHistogramsSib[ptBin][centralityClass]->Fill(kPiPiMass);
                            dStarMinusD0HistogramsSib[ptBin][centralityClass]->Fill(dStarPionMass);    //  for D*  

                            if(dStarPionMass > .143 && dStarPionMass < .147){     //changed from .140, .149 -- too wide
                            
                                if(USE_SYMMETERIZATION){
                                    dStarAngularDistSib[ptBin][centralityClass]->Fill(delEta, delPhi, finalPairWeight);
                                    dStarAngularDistSib[ptBin][centralityClass]->Fill(-delEta, delPhiCp, finalPairWeight);
                                    dStarAngularDistSib[ptBin][centralityClass]->Fill(-delEta, delPhi, finalPairWeight);
                                    dStarAngularDistSib[ptBin][centralityClass]->Fill(delEta, delPhiCp, finalPairWeight);
                                }
                                else dStarAngularDistSib[ptBin][centralityClass]->Fill(delEtaRaw, delPhiRaw, finalPairWeight);
                            }
                        ...
                        if(ptBin > -1) { 
                            ...
                            else sibCorrBinPt[bandBin][ptBin][VzBin][centralityBin]->Fill(delEtaRaw, delPhiRaw, finalPairWeight);
                        }
                    }
                    ...
                    for (int idx = 0; idx < nD0Tracks; idx++){//begin loop over event D0 kaon-pion list
                        ...
                        if(kaonPionTrack1.charge() < 0 && kaonPionTrack1.mass() > D0InvMassLow && kaonPionTrack1.mass() < D0InvMassHigh)                 { bandBin = 1; }    //US center band
                            ...
                            for(int j = 0; j < nHadronTracks; j++){//begin loop over hadron event tracks
                                ...
                                delPhiRaw = kaonPionTrack1.gMom().phi()-phi;
                                delEtaRaw = kaonPionTrack1.gMom().pseudoRapidity()-eta;
                                ...
                                if(!USE_PAIR_WEIGHT) { finalPairWeight = 1; }
                                 ...
                                 kPiPiMass = calcInvMass3Particle(.493677, .139570 , .139570 , kaonPionTrack1.kaonGMom().x(), kaonPionTrack1.kaonGMom().y(), kaonPionTrack1.kaonGMom().z(), 
                                                          kaonPionTrack1.pionGMom().x(), kaonPionTrack1.pionGMom().y(), kaonPionTrack1.pionGMom().z(), trackMom.x(), trackMom.y(), trackMom.z()); 
                        
                                dStarPionMass = kPiPiMass - kaonPionTrack1.mass();
                                
                                if(bandBin == 1 && hadronTrack2.particleType() == 0 && ((kaonPionTrack1.pionCharge()*hadronTrack2.charge()) == 1) 
                                                && dStarPionMass > 0.0 && dStarPionMass < .5 && kPiPiMass > 1.6 && kPiPiMass < 3.0 ){
                                    
                                    
                                    dStarHistogramsMix[ptBin][centralityClass]->Fill(kPiPiMass);
                                    dStarMinusD0HistogramsMix[ptBin][centralityClass]->Fill(dStarPionMass);    //  for D* 
                                    

                                    if(dStarPionMass > .143 && dStarPionMass < .147){
                                    
                                        if(USE_SYMMETERIZATION){ 
                                            dStarAngularDistMix[ptBin][centralityClass]->Fill(delEta, delPhi, finalPairWeight);
                                            dStarAngularDistMix[ptBin][centralityClass]->Fill(-delEta, delPhiCp, finalPairWeight);
                                            dStarAngularDistMix[ptBin][centralityClass]->Fill(-delEta, delPhi, finalPairWeight);
                                            dStarAngularDistMix[ptBin][centralityClass]->Fill(delEta, delPhiCp, finalPairWeight);
                                        }
                                        else dStarAngularDistMix[ptBin][centralityClass]->Fill(delEtaRaw, delPhiRaw, finalPairWeight);
                                    }
                                ...
                                if(D0_HADRON_CORR){
                                    ...
                                    if(ptBin > -1) { 
                                        ...
                                        else mixCorrBinPt[bandBin][ptBin][VzBin][centralityBin]->Fill(delEtaRaw, delPhiRaw, finalPairWeight);
                                    }
                                }
...
int StPicoD0AnaMaker::getCentralityBin(int nTracks){
    ...
    if(nTracks >= 1   && nTracks < 42)  { return 0;  } //1
    if(nTracks >= 42  && nTracks < 86)  { return 1;  } //2
    if(nTracks >= 86  && nTracks < 131)  { return 2;  } //3
    
    if(nTracks >= 131  && nTracks < 183) { return 3;  } //4
    if(nTracks >= 183 && nTracks < 235) { return 4;  }  //5
    if(nTracks >= 235 && nTracks < 288) { return 5;  }  //6
    if(nTracks >= 288 && nTracks < 340) { return 6;  }  //7
    if(nTracks >= 340 && nTracks < 392) { return 7;  }  //8
    if(nTracks >= 392 && nTracks < 440) { return 8;  }  //9
    
    if(nTracks >= 440 && nTracks < 491) { return 9;  }  //10
    if(nTracks >= 491 && nTracks < 542) { return 10;  } //11
    if(nTracks >= 542 && nTracks < 593) { return 11;  } //12
    if(nTracks >= 593 && nTracks < 644) { return 12;  } //13
    if(nTracks >= 644 && nTracks < 695) { return 13;  } //14
    if(nTracks >= 695 && nTracks < 746) { return 14;  } //15
    if(nTracks >= 746)                  { return 15; }  //16
...
int StPicoD0AnaMaker::getPtBin(double pt){
    ...
    //if(pt >=  3.0  && pt <  4.0)   { return 3;  }   //changed to 3-4
    //if(pt >=  4.0  && pt <  10.0)   { return 4;  }  //changed to 4-10
...
double StPicoD0AnaMaker::calcInvMass(double mass1, double mass2, double p1x, double p1y, double p1z, double p2x, double p2y, double p2z){
...
double StPicoD0AnaMaker::calcInvMass3Particle(double mass1, double mass2, double mass3, double p1x, double p1y, double p1z, double p2x, double p2y, double p2z, double p3x, double p3y, double p3z){

    double massTerm   = (mass1*mass1)+(mass2*mass2)+(mass3*mass3);
    double energyTerm = (2*TMath::Sqrt((mass1*mass1)+(p1x*p1x+p1y*p1y+p1z*p1z))*TMath::Sqrt((mass2*mass2)+(p2x*p2x+p2y*p2y+p2z*p2z))) 
                      + (2*TMath::Sqrt((mass2*mass2)+(p2x*p2x+p2y*p2y+p2z*p2z))*TMath::Sqrt((mass3*mass3)+(p3x*p3x+p3y*p3y+p3z*p3z))) 
                      + (2*TMath::Sqrt((mass3*mass3)+(p3x*p3x+p3y*p3y+p3z*p3z))*TMath::Sqrt((mass1*mass1)+(p1x*p1x+p1y*p1y+p1z*p1z)));
    
    double momTerm    = (2*(p1x*p2x+p1y*p2y+p1z*p2z)) + (2*(p1x*p3x+p1y*p3y+p1z*p3z)) + (2*(p2x*p3x+p2y*p3y+p2z*p3z));

    if((massTerm + energyTerm - momTerm) < 0) { return 1.6; }
    
    else return TMath::Sqrt(massTerm + energyTerm - momTerm);
