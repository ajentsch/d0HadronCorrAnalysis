...
int D0CorrelationStatErrorMaker_ver2(){
    ...
    TFile * inputFile = new TFile("singles_dists_pt_2_10Gev.root");
    TFile * outputFile = new TFile("output_errors_correct_2_10GeV.root", "RECREATE");
    
    TString vzLabel          = "_VzBin_";
    TString multLabel        = "_CentBin_";
    TString siblingLabel     = "Sibling__";
    TString ptBinLabelName   = "PtBin_";
    TString bandLabel[3]     = {"SBL_", "US_Signal_Region_", "SBR_"};  //FOR THE ORIGINAL ERRORS, I MIXED UP LEFT AND RIGHT
        ...
        for(int ptBin = 0; ptBin < NUM_PT_BINS; ptBin++){
            for(int vzBin = 0; vzBin < 10; vzBin++){
                for(int multBin = 0; multBin < 16; multBin++){
                    ...
                    str1 = errorHistLabel + bandLabel[band] + ptBinLabelName + ptBinLabel[ptBin] + vzLabel + vzBinLabel[vzBin] + multLabel + multBinLabel[multBin];
    ...
    for(int i = 1; i < NUM_PHI_PRIM_BINS+1; i++){
        ...
        double etaZValues[10][10][10];
        double phiZValues[13][25][25];  //1 more than needed to keep indexing the same as histo-bins
       ...
       /*  bool oneOrOneHalf = false; // false -- 1, true -- 1/2
        ...
        for(int delEtaBin = 1; delEtaBin < 10; delEtaBin++){
            for(int delPhiBin = 1; delPhiBin < 13; delPhiBin++){   
                ...
                delPhiValue = delPhiCorrBinCenters[delPhiBin]; 
                    ...
                    for(int etaHBin = 1; etaHBin < NUM_ETA_PRIM_BINS+1; etaHBin++){
                        for(int phiDBin = 1; phiDBin < NUM_PHI_PRIM_BINS+1; phiDBin++){
                            for(int phiHBin = 1; phiHBin < NUM_PHI_PRIM_BINS+1; phiHBin++){
                                ...
                                if( (isDiffZero(delEtaStripe, delEtaValue, .0001)) || (isDiffZero(delEtaStripeMirror, delEtaValue, .0001)) ) {
                                
                                    etaZValues[delEtaBin][etaDBin][etaHBin] = 1.0;
                                    
                                }
                                   
                                else if( (isDiffZero((delEtaStripe - .5*delEtaBinWidthCorr), delEtaValue, .0001)) || (isDiffZero((delEtaStripe + .5*delEtaBinWidthCorr), delEtaValue, .0001)) ||
                                         (isDiffZero((delEtaStripeMirror - .5*delEtaBinWidthCorr), delEtaValue, .0001)) || (isDiffZero((delEtaStripeMirror + .5*delEtaBinWidthCorr), delEtaValue, .0001))) {
                                   
                                    etaZValues[delEtaBin][etaDBin][etaHBin] = 0.5;   
                                    
                                }    
                                 
                                else etaZValues[delEtaBin][etaDBin][etaHBin] = 0.0;
                                ...
                                if((delPhiStripe == delPhiValue) || (delPhiStripeMirror == delPhiValue)  ) {
                                
                                    phiZValues[delPhiBin][phiDBin][phiHBin] = 1.0;
                                        
                                }
                                   
                                   
                                else if( ((delPhiStripe - 2.0) == delPhiValue) || (((delPhiStripe + 2.0) == delPhiValue)) ||
                                    ((delPhiStripeMirror - 2.0) == delPhiValue) || (((delPhiStripeMirror + 2.0) == delPhiValue)) ) {
                                    
                                    phiZValues[delPhiBin][phiDBin][phiHBin] = 0.5;    
                                
                                }
       
                                
                                else phiZValues[delPhiBin][phiDBin][phiHBin] = 0.0; 

                                //if(etaZValues[delEtaBin][etaDBin][etaHBin] != 0) { cout << "Eta D: " << etaDBin << "   EtaH: " << etaHBin << "     Z = " << etaZValues[delEtaBin][etaDBin][etaHBin] << endl; }
        ...
        cout << endl << "Z-values calculated and stored." << endl << endl;
        ...
        for(int ptBin = 2; ptBin < NUM_PT_BINS; ptBin++){
            for(int vzBin = 0; vzBin < 10; vzBin++){
                for(int multBin = 0; multBin < 16; multBin++){
                    ...
                    cout << "Calculating errors for band " << band << ", ptBin " << ptBin << ", vzBin " << vzBin << " , and multBin " << multBin << endl;
                
                    for(int delEtaBin = 1; delEtaBin < 10; delEtaBin++){
                        for(int delPhiBin = 1; delPhiBin < 13; delPhiBin++){       //only loop over main bins, symmeterized at the end.
                                ...
                                for(int etaHBin = 1; etaHBin < NUM_ETA_PRIM_BINS+1; etaHBin++){
                                    ...
                                    for(int phiDBin = 1; phiDBin < NUM_PHI_PRIM_BINS+1; phiDBin++){
                                        for(int phiHBin = 1; phiHBin < NUM_PHI_PRIM_BINS+1; phiHBin++){
                                            ...
                                            if(etaZValues[delEtaBin][etaDBin][etaHBin] == 0 || phiZValues[delPhiBin][phiDBin][phiHBin] == 0) { continue; }
                                            ...
                                            term1 = term1 + NEv[band][ptBin][vzBin][multBin]*NMix*(1+NMix)*etaZValues[delEtaBin][etaDBin][etaHBin]*etaZValues[delEtaBin][etaDBin][etaHBin]*phiZValues[delPhiBin][phiDBin][phiHBin]*phiZValues[delPhiBin][phiDBin][phiHBin]*nBar*mBar*(1-mBar);
                                            ...
                                            for(int etaDBinPrime = 1; etaDBinPrime < NUM_ETA_PRIM_BINS+1; etaDBinPrime++){
                                                for(int phiDBinPrime = 1; phiDBinPrime < NUM_PHI_PRIM_BINS+1; phiDBinPrime++){
                                                    ...
                                                    nBarPrime = nBarHist[band][ptBin][vzBin][multBin]->GetBinContent(etaHBin, phiHBin)/NEv[band][ptBin][vzBin][multBin];
        
                                                    //if(mBarPrime == 0 || nBarPrime == 0) { continue; }
                
                                                    term2 = term2 + 2*NEv[band][ptBin][vzBin][multBin]*NMix*NMix*etaZValues[delEtaBin][etaDBinPrime][etaHBin]*phiZValues[delPhiBin][phiDBinPrime][phiHBin]*etaZValues[delEtaBin][etaDBinPrime][etaHBin]*phiZValues[delPhiBin][phiDBinPrime][phiHBin]*mBar*mBarPrime*nBar;
                                ...
                                for(int etaHBin = 1; etaHBin < NUM_ETA_PRIM_BINS+1; etaHBin++){
                                    for(int phiDBin = 1; phiDBin < NUM_PHI_PRIM_BINS+1; phiDBin++){
                                        for(int phiHBin = 1; phiHBin < NUM_PHI_PRIM_BINS+1; phiHBin++){
                                            ...
                                            denominator = denominator + NEv[band][ptBin][vzBin][multBin]*NMix*etaZValues[delEtaBin][etaDBin][etaHBin]*phiZValues[delPhiBin][phiDBin][phiHBin]*mBar*nBar;
                            ...
                            if(term1 == 0.0 && term2 == 0.0) {continue;}
                           
                            denominator = denominator*denominator;
                           
                            ////////////////////Don't change anything past here for now. This is the calculation of the final error after all the internal loops for a delEta, delPhi bin execute
