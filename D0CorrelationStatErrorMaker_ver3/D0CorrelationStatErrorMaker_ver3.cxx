...
int D0CorrelationStatErrorMaker_ver3(){
    ...
    TFile * inputFile = new TFile("best_output_for_stat_errors_BEST_FILE_3.root");
    TFile * outputFile = new TFile("output_errors_correct_FULL.root", "RECREATE");
        ...
        for(int ptBin = 2; ptBin < 5; ptBin++){
            for(int vzBin = 0; vzBin < 10; vzBin++){
                for(int multBin = 0; multBin < 16; multBin++){
                    ...
                    cout << "Band, PtBin, VzBin, MultBin: " << band << " , " << ptBin << " , " << vzBin << " , " << multBin << endl; 
                        ...
                        for(int delPhiBin = 1; delPhiBin < 13; delPhiBin++){       //only loop over main bins, symmeterized at the end.
                            ...
                            for(int etaDBin = 1; etaDBin < NUM_ETA_PRIM_BINS+1; etaDBin++){
                                for(int etaHBin = 1; etaHBin < NUM_ETA_PRIM_BINS+1; etaHBin++){
                                    ...
                                    if(etaZValues[delEtaBin][etaDBin][etaHBin] == 0) { continue; }
                                    numPhiBins = 0;
                                
                                    for(int phiDBin = 1; phiDBin < NUM_PHI_PRIM_BINS+1; phiDBin++){
                                        for(int phiHBin = 1; phiHBin < NUM_PHI_PRIM_BINS+1; phiHBin++){
                                        
                                            if(phiZValues[delPhiBin][phiDBin][phiHBin] == 0) { continue; }
