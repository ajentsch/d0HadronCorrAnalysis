...
int D0corrFittingAndPlotMaker(){
   ...
   // TH2D* errorOnBin[3];
    ...
    //                               A0      AD     AQ    A1   sPhi sEta  
        ...
        myfit[2][0]->SetParLimits(1, -.03,  0.0);    //v1     AD
        myfit[2][0]->SetParLimits(2, 0.0,  .03);    //v2     AQ
        myfit[2][0]->SetParLimits(3, 0.01, 0.1);    //A1 jet amp
        myfit[2][0]->SetParLimits(4, 0.1, 1.0);     //phi width
        myfit[2][0]->SetParLimits(5, 0.1, .6);    //eta width
        
         //                         A0      AD     AQ    A1   sPhi sEta  
        //myfit[2][1]->SetParameters(-.005, -.01,   .01,  .03,  .7, 1.4);
        
        myfit[2][1]->SetParLimits(0, -.02, 0.0);    //offset A0
        myfit[2][1]->SetParLimits(1, -.015,  0.0);    //v1     AD
        myfit[2][1]->SetParLimits(2, 0.0,  .03);    //v2     AQ
        myfit[2][1]->SetParLimits(3, 0.0, .03);    //A1 jet amp
        myfit[2][1]->SetParLimits(4, 0.9, 1.5);     //phi width
        myfit[2][1]->SetParLimits(5, 0.8, 1.8);    //eta width
        
        //                         A0      AD     AQ    A1   sPhi sEta  
        //myfit[2][2]->SetParameters(-.0007, -.002, .003, .02,  .32, .37);
        
        myfit[2][2]->SetParLimits(0, -.005, 0.0);    //offset A0
        myfit[2][2]->SetParLimits(1, -.01,  0.0);    //v1     AD
        myfit[2][2]->SetParLimits(2, 0.0,  .01);    //v2     AQ
        myfit[2][2]->SetParLimits(3, 0.0, .02);    //A1 jet amp
        myfit[2][2]->SetParLimits(4, 0.0, 1.5);     //phi width
        myfit[2][2]->SetParLimits(5, 0.0, 2.5);    //eta width 
    
    
        //////////////////////////////////////////////////////////////////////////////////////
        myfit[3][0]->SetParLimits(0, -.01, 0.0);    //offset A0
        myfit[3][0]->SetParLimits(1, -.02,  0.0);    //v1     AD
        myfit[3][0]->SetParLimits(2, 0.0,  .02);    //v2     AQ
        myfit[3][0]->SetParLimits(3, 0.0, .2);    //A1 jet amp
        myfit[3][0]->SetParLimits(4, 0.1, .8);     //phi width
        myfit[3][0]->SetParLimits(5, 0.1, .8);    //eta width
        
        myfit[3][1]->SetParLimits(0, -.02, .01);    //offset A0
        myfit[3][1]->SetParLimits(1, -.05,  0.0);    //v1     AD
        myfit[3][1]->SetParLimits(2, 0.0,  .05);    //v2     AQ
        myfit[3][1]->SetParLimits(3, 0.0, .2);    //A1 jet amp
        myfit[3][1]->SetParLimits(4, 0.0, 1.5);     //phi width
        myfit[3][1]->SetParLimits(5, 0.0, 2.0);    //eta width
        
        myfit[3][2]->SetParLimits(0, -.01, 0.0);    //offset A0
        myfit[3][2]->SetParLimits(1, -.01,  0.0);    //v1     AD
        myfit[3][2]->SetParLimits(2, 0.0,  .01);    //v2     AQ
        myfit[3][2]->SetParLimits(3, 0.0, .04);    //A1 jet amp
        myfit[3][2]->SetParLimits(4, 0.0, 1.5);     //phi width
        myfit[3][2]->SetParLimits(5, 0.0, 2.0);    //eta width
        ///////////////////////////////////////////////////////////////////////////////////////
        myfit[4][0]->SetParLimits(0, -.01, .0.0);    //offset A0
        myfit[4][0]->SetParLimits(1, -.03,  0.0);    //v1     AD
        myfit[4][0]->SetParLimits(2, 0.0,  .01);    //v2     AQ
        myfit[4][0]->SetParLimits(3, 0.0, .17);    //A1 jet amp
        myfit[4][0]->SetParLimits(4, 0.1, .8);     //phi width
        myfit[4][0]->SetParLimits(5, 0.1, .8);    //eta width
        
        myfit[4][1]->SetParLimits(0, -.02, 0.0);    //offset A0
        myfit[4][1]->SetParLimits(1, -.02,  0.0);    //v1     AD
        myfit[4][1]->SetParLimits(2, 0.0,  .02);    //v2     AQ
        myfit[4][1]->SetParLimits(3, -0.05, .05);    //A1 jet amp
        myfit[4][1]->SetParLimits(4, 0.1, 1.0);     //phi width
        myfit[4][1]->SetParLimits(5, 0.1, 1.0);    //eta width
        
        myfit[4][2]->SetParLimits(0, -.04, .01);    //offset A0
        myfit[4][2]->SetParLimits(1, -.05,  0.0);    //v1     AD
        myfit[4][2]->SetParLimits(2, 0.0,  .02);    //v2     AQ
        myfit[4][2]->SetParLimits(3, 0.0, .06);    //A1 jet amp
        myfit[4][2]->SetParLimits(4, 0.1, .7);     //phi width
        myfit[4][2]->SetParLimits(5, 0.1, .9);    //eta width
