...
int D0corrFittingAndPlotMaker(){
    ...
    bool USE_SIX_PARAMETER_STANDARD     = true;                  //currently using for mid-central
    bool USE_TWO_2D_GAUSSIANS           = false;
    bool USE_TWO_2D_GAUSSIANS_PLUS_QUAD = false;
    bool USE_2D_SS_GAUSS_AS_GAUSS_WITH_DIPOLE_PLUS_QUAD = false;   //CURRENTLY USING FOR CENTRAL BIN
    ...
    int NUM_PHI_BINS = 12;
    int NUM_ETA_BINS = 13;
    double ETA_RANGE = 1.38;
    double phiBinShift = (TMath::Pi()/12.0);
    
    double v2Array[5][3];
    double v2ArrayStatError[5][3];

    TString inputFileName  = "d0HadronCorrMaker_OUTPUT_dStar_correction_with_no_pion_PID_12_phi_13_eta_2_10_pt_20PercentChange_to_dStar_norm.root";
	TString outputFileName = "fittingOutput_OUTPUT_dStar_correction_with_no_pion_PID_12_phi_13_eta_2_10_pt_20PercentChange_to_dStar_norm.root";
   ...
   // TH2D* errorOnBin[3];
    ...
    for(int k = 2; k < 5; k++){
        for(int i = 0; i < 3; i++){ 
			...
			if(AS_ONLY_FIT_CENTRAL) { myfit[k][i] = new TF2(str1,"[0] + 2*[1]*cos(2*y) +  [2]*exp(-0.5*(x*x)/([3]*[3]))*(exp(-0.5*((y-3.14159)*(y-3.14159))/([4]*[4]))+exp(-0.5*((y+3.14159)*(y+3.14159))/([4]*[4])))", -ETA_RANGE, ETA_RANGE, TMath::PiOver2()+phiBinShift, 3*TMath::PiOver2()+phiBinShift); }
    ...
    if(USE_SIX_PARAMETER_STANDARD) { //use for mid-central
       ...
       // myfit[2][1]->SetParameters(-.012, -.0208,   .00478,  .0325,  .67,    4.0);
        ...
        //myfit[2][1]->SetParLimits(1, -.03, -0.016);    //v1     AD
        //myfit[2][1]->SetParLimits(2, 0.005,  .009);    //v2     AQ
        myfit[2][1]->FixParameter(1, -.0177424); //AD
        myfit[2][1]->FixParameter(2, .00605651); //v2 AQ
        ...
        myfit[2][2]->SetParLimits(0, -.012, 0.0);    //offset A0
        myfit[2][2]->SetParLimits(1, -.02,  0.0);    //v1     AD
        myfit[2][2]->SetParLimits(2, 0.0005,  .007);    //v2     AQ
        //myfit[2][2]->FixParameter(2, .0036); //v2 AQ
        myfit[2][2]->SetParLimits(3, 0.0, .04);    //A1 jet amp
    ...
    //                               A0      AQ    A1    JetPhi  JetEta    ASAmp   ASPhi  ASEta  
        ...
        myfit[2][0]->SetParLimits(0, -.01, 0.0);    //offset  
        myfit[2][0]->SetParLimits(1, 0.006, .01);    //quadrupole  
        myfit[2][0]->SetParLimits(2, 0.07,  .1);   //jet amp
        ...
        myfit[2][0]->SetParLimits(6, 0.5, 1.2);    //AS phi width
        myfit[2][0]->SetParLimits(7, 1.0, 10.0);    //AS eta width
    ...
    //                               A0      AQ    A1    JetPhi  JetEta    ASAmp   ASPhi  ASEta  
        ...
        //myfit[2][2]->SetParameters(-.001, .00305,  .004,  .4,  2.0,     -.006       ,   .4      );
        
        myfit[2][2]->SetParLimits(0, -.015, 0.0);    //offset  
        myfit[2][2]->SetParLimits(1, 0.00, .004);    //quadrupole  
        myfit[2][2]->SetParLimits(2, 0.008,  .19);   //jet amp
        myfit[2][2]->SetParLimits(3, 0.3, .8);    //jet phi width
        myfit[2][2]->SetParLimits(4, .5, 1.5);    //jet eta width 
        myfit[2][2]->SetParLimits(5, -0.02, 0.0);    //AS dipole
        myfit[2][2]->SetParLimits(6, 0.05, 1.5);    //AS eta width
    ...
    if(OFFSET_QUAD_JETPEAK_AS_1D_GAUSS){ //use for peripheral
        ...
        myfit[2][0]->SetParameters(-0.011, .005, .11, .3, .27, .028, .52);
        
        myfit[2][0]->SetParLimits(0, -.015, -0.002);    //offset A0
        myfit[2][0]->SetParLimits(1, 0.003,  0.01);    //v2     AQ
        myfit[2][0]->SetParLimits(2, 0.01, .14);    //A1 jet amp
        myfit[2][0]->SetParLimits(3, 0.195, .34);     //phi width
        myfit[2][0]->SetParLimits(4, 0.24, .30);    //eta width 
    ...
    if(AS_ONLY_FIT_CENTRAL) {
        ...
        myfit[2][2]->SetParLimits(1, 0.002,  0.01);    //v2     AQ
        myfit[2][2]->SetParLimits(2, 0.0, 0.01);    //AS amp
        myfit[2][2]->SetParLimits(3, 0.2, 1.2);     //AS eta width
		myfit[2][2]->SetParLimits(4, 0.7, 1.2);     //AS phi width
    ...
    finalCorrGridCanvas->SaveAs("nominal_12_phi_bins_13_eta_pt_2_10.png");
    ...
    TString outputFitString = "Fitting_pt_2_10_GeV_nominal_cuts_PtBin_";
    
    double sigmaValueSquare, sigmaValue;
    double data, model, error;
    double delEtaBinCenter, delPhiBinCenter;
    double maxZ, minZ;
    TString chiPlotLabel = "Plot_of_chi_values_per_bin_";
    ...
    for(int k = FIRST_PT_BIN; k < NUM_PT_BINS; k++){
        ...
        for(int i = 0; i < 3; i++){
           ...
           //cout << "CHECK IT!" << endl;
            ...
            str1 = chiPlotLabel + ptBinLabel[k];
    ...
    TString outputPhiProjString = "phiProjection_pt_2_10_GeV_nominal_cuts_";
   ...
   // finalFitCorrGridCanvas->SaveAs("Fitting_testing_for_final_results_symmeterized_April_2018.png");
    ...
    TString fitParameterLabel = "FIT_PARAMETERS_pt_2_10_GeV_nominal_cuts_PtBin_";
