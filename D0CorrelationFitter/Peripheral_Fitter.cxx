...
int Peripheral_Fitter(){
	...
	TString outputFileName = "fittingOutput_pt_2_10_GeV_nominal_cuts.root";
   ...
   // TH2D* errorOnBin[3];
    ...
    for(int i = MIN_CENT_BIN; i < MAX_CENT_BIN; i++){
    ...
    for(int k = 2; k < 5; k++){
        for(int i = MIN_CENT_BIN; i < MAX_CENT_BIN; i++){ 
            ...
            if(USE_TWO_2D_GAUSSIAN_SS_1D_GAUSS_AS_PLUS_QUAD) { myfit[k][i] = new TF2(str1,"[0] + 2*[1]*cos(2*y) + [2]*(exp(-0.5*(y*y)/([3]*[3])) + exp(-0.5*((y-6.28)*(y-6.28))/([3]*[3])))*exp(-0.5*(x*x)/([4]*[4])) + [5]*(exp(-0.5*((y-3.14159)*(y-3.14159))/([6]*[6]))+exp(-0.5*((y+3.14159)*(y+3.14159))/([6]*[6])))",-ETA_RANGE, ETA_RANGE, -TMath::PiOver2()+phiBinShift, 3*TMath::PiOver2()+phiBinShift);}
        }
	}
    ...
    //                               A0      AD     AQ    A1   sPhi sEta  
        ...
        myfit[2][0]->SetParLimits(1, -.02,  0.0);    //v1     AD
        myfit[2][0]->SetParLimits(2, 0.0,  .03);    //v2     AQ
        //myfit[2][0]->FixParameter(2, .0084); //v2 AQ
        myfit[2][0]->SetParLimits(3, 0.09, 0.15);    //A1 jet amp
        myfit[2][0]->SetParLimits(4, 0.1, 1.0);     //phi width
        myfit[2][0]->SetParLimits(5, 0.1, 1.2);    //eta width
    ...
    //                               A0      AQ    A1    JetPhi  JetEta    ASAmp   ASPhi  ASEta  
        ...
        myfit[2][0]->SetParLimits(3, 0.0, 1.0);    //jet phi width
        myfit[2][0]->SetParLimits(4, 0.0, 1.0);    //jet eta width 
        myfit[2][0]->SetParLimits(5, 0.0, .03);    //AS amp
        myfit[2][0]->SetParLimits(6, 0.0, 1.4);    //AS phi width
        myfit[2][0]->SetParLimits(7, 0.0, 2.2);    //AS eta width
        
    }
    
    if(USE_TWO_2D_GAUSSIAN_SS_1D_GAUSS_AS_PLUS_QUAD) {
    //                               A0      AQ    A1    JetPhi  JetEta    ASAmp   ASPhi  ASEta  
        //myfit[2][0]->SetParameters(-.003,  .01,   .8,    .6,      .5,     .02,     .9,    1.2);
    
        myfit[2][0]->SetParLimits(0, -.03, 0.0);    //offset  
        myfit[2][0]->SetParLimits(1, 0.0045, .0055);    //quadrupole  
        myfit[2][0]->SetParLimits(2, 0.10,  .13);   //jet amp
        myfit[2][0]->SetParLimits(3, 0.1, .4);    //jet phi width
        myfit[2][0]->SetParLimits(4, 0.1, .4);    //jet eta width 
        myfit[2][0]->SetParLimits(5, 0.0, .04);    //AS amp
        myfit[2][0]->SetParLimits(6, 0.1, .6);    //AS phi width
   ...
   // finalFitCorrGridCanvas->SaveAs("Fitting_testing_for_final_results_symmeterized_April_2018.png");
    ...
    TCanvas * finalFitParametersCorrGridCanvas = new TCanvas("", "", 3600, 3000);
