...
//------------------------------------------
 ...
 //------------------------------------------------------------------------------------------------------------------------------------
   ...
   //-----------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    for(int k = 3; k < 4; k++){
        for(int i = 0; i < 3; i++){ 
            ...
            str1 = fitFunctionLabel + PtBinLabel[k] + binLabelPt[k] + centralityBin[i];
            myfit[k][i] = new TF2(str1,"[0] + [1]*cos(y) + [2]*2*cos(2*y) + [3]*(1./TMath::Beta(0.5,0.5*r)*TMath::Sqrt(lambda/r)*TMath::Power(1.+lambda*(x[0]-mu)*(x[0]-mu)/r, -0.5-0.5*r))", -ETA_RANGE, ETA_RANGE,-1.57,4.71);
		}
	}*/
    ...
    myfit[0][0]->SetParLimits(3,  0.0,  .19);    //Jetamp A1
    myfit[0][0]->SetParLimits(4, 0.1, 2);     //sigPhi
    myfit[0][0]->SetParLimits(5, 0.1, 2);    //sigEta
	
	myfit[0][1]->SetParLimits(0, -.1, .01);    //offset A0
    myfit[0][1]->SetParLimits(1, -.05,  0.0);    //v1     AD
    myfit[0][1]->SetParLimits(2, 0.0,  .1);    //v2     AQ
    myfit[0][1]->SetParLimits(3,  0.0,  .19);    //Jetamp A1
    myfit[0][1]->SetParLimits(4, 0.1, 2);     //sigPhi
    myfit[0][1]->SetParLimits(5, 0.1, 2);    //sigEta
	
	myfit[0][2]->SetParLimits(0, -.1, .01);    //offset A0
    myfit[0][2]->SetParLimits(1, -.05,  0.0);    //v1     AD
    myfit[0][2]->SetParLimits(2, 0.0,  .1);    //v2     AQ
    myfit[0][2]->SetParLimits(3,  0.0,  .19);    //Jetamp A1
    myfit[0][2]->SetParLimits(4, 0.1, 2);     //sigPhi
    myfit[0][2]->SetParLimits(5, 0.1, 2);    //sigEta
	
	myfit[1][0]->SetParLimits(0, -.1, .01);    //offset A0
    myfit[1][0]->SetParLimits(1, -.05,  0.0);    //v1     AD
    myfit[1][0]->SetParLimits(2, 0.0,  .1);    //v2     AQ
    myfit[1][0]->SetParLimits(3,  0.0,  .19);    //Jetamp A1
    myfit[1][0]->SetParLimits(4, 0.1, 2);     //sigPhi
    myfit[1][0]->SetParLimits(5, 0.1, 2);    //sigEta
	
	myfit[1][1]->SetParLimits(0, -.1, .01);    //offset A0
    myfit[1][1]->SetParLimits(1, -.05,  0.0);    //v1     AD
    myfit[1][1]->SetParLimits(2, 0.0,  .1);    //v2     AQ
    myfit[1][1]->SetParLimits(3,  0.0,  .07);    //Jetamp A1
    myfit[1][1]->SetParLimits(4, 0.1, 2);     //sigPhi
    myfit[1][1]->SetParLimits(5, 0.1, 2);    //sigEta
	
	myfit[1][2]->SetParLimits(0, -.1, .01);    //offset A0
    myfit[1][2]->SetParLimits(1, -.05,  0.0);    //v1     AD
    myfit[1][2]->SetParLimits(2, 0.0,  .1);    //v2     AQ
    myfit[1][2]->SetParLimits(3,  0.0,  .04);    //Jetamp A1
    myfit[1][2]->SetParLimits(4, 0.1, 2);     //sigPhi
    myfit[1][2]->SetParLimits(5, 0.1, 2);    //sigEta
	
	myfit[2][0]->SetParLimits(0, -.1, .01);    //offset A0
    myfit[2][0]->SetParLimits(1, -.06,  0.0);    //v1     AD
    myfit[2][0]->SetParLimits(2, 0.0,  .1);    //v2     AQ
    myfit[2][0]->SetParLimits(3,  0.0,  .19);    //Jetamp A1
    myfit[2][0]->SetParLimits(4, 0.1, 2);     //sigPhi
    myfit[2][0]->SetParLimits(5, 0.1, 2);    //sigEta
	
	myfit[2][1]->SetParLimits(0, -.06, 0.0);    //offset A0
    myfit[2][1]->SetParLimits(1, -.06,  0.0);    //v1     AD
    myfit[2][1]->SetParLimits(2, 0.0,  .04);    //v2     AQ
    myfit[2][1]->SetParLimits(3,  0.0,  .15);    //Jetamp A1
    myfit[2][1]->SetParLimits(4, 0.2, .9);     //sigPhi
    myfit[2][1]->SetParLimits(5, 0.2, .7);    //sigEta
	
	myfit[2][2]->SetParLimits(0, -.1, .01);    //offset A0
    myfit[2][2]->SetParLimits(1, -.05,  0.0);    //v1     AD
    myfit[2][2]->SetParLimits(2, 0.0,  .1);    //v2     AQ
    myfit[2][2]->SetParLimits(3,  0.0,  .09);    //Jetamp A1
    myfit[2][2]->SetParLimits(4, 0.1, 2);     //sigPhi
    myfit[2][2]->SetParLimits(5, 0.1, 2);    //sigEta
	
	myfit[3][0]->SetParLimits(0, -.04, 0.0);    //offset A0
    myfit[3][0]->SetParLimits(1, -.03,  0.0);    //v1     AD
    myfit[3][0]->SetParLimits(2, 0.0,  .03);    //v2     AQ
    myfit[3][0]->SetParLimits(3,  0.0,  .17);    //Jetamp A1
    myfit[3][0]->SetParLimits(4, 0.1, 2);     //sigPhi
    myfit[3][0]->SetParLimits(5, 0.1, 2);    //sigEta
	
	myfit[3][1]->SetParLimits(0, -.03, 0.0);    //offset A0
    myfit[3][1]->SetParLimits(1, -.02,  0.0);    //v1     AD
    myfit[3][1]->SetParLimits(2, 0.0,  .02);    //v2     AQ
    myfit[3][1]->SetParLimits(3,  0.0,  .08);    //Jetamp A1
    myfit[3][1]->SetParLimits(4, 0.1, 2);     //sigPhi
    myfit[3][1]->SetParLimits(5, 0.1, 2);    //sigEta
	
	myfit[3][2]->SetParLimits(0, -.02, 0.0);    //offset A0
    myfit[3][2]->SetParLimits(1, -.02,  0.0);    //v1     AD
    myfit[3][2]->SetParLimits(2, 0.0,  .02);    //v2     AQ
    myfit[3][2]->SetParLimits(3,  0.0,  .05);    //Jetamp A1
    myfit[3][2]->SetParLimits(4, 0.1, 2);     //sigPhi
    myfit[3][2]->SetParLimits(5, 0.1, 2);    //sigEta
   ...
   /////////////////////////////////////
    ...
    TString dataPointsForLannyFile = "Data_Points_PtBin2_MidCentral.txt";
    ...
    for(int etaBin = 2; etaBin < 9; etaBin++){
        for(int phiBin = 1; phiBin < 13; phiBin++){
            ...
            dataPointsForLanny << (etaBin-1) << "\t" << phiBin << "\t" << fullySubtractedCorrCustomCentralitySideBand[2][1]->GetBinContent(etaBin, phiBin) << "\t" <<
            fullySubtractedCorrCustomCentralitySideBand[2][1]->GetBinError(etaBin, phiBin) << endl;
        }
    }        
